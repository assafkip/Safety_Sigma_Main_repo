...
def render_behavioral_sections(parts: list[str]):
    """Render Behavioral Analysis and Behavioral Families sections."""
    esc = html.escape
    
    # Behavioral Analysis - current case behaviors
    parts.append("""<div class="section">
  <h2>🎭 Behavioral Analysis</h2>
  <p class="small">Literal behavioral phrases extracted from this case with zero inference. Shows tactics, payment methods, and communication patterns.</p>
""")
    
    # Load behavioral indicators if available
    behaviors_path = ART / "behaviors" / "extracted_behaviors.json"
    current_behaviors = []
    
    if behaviors_path.exists():
        try:
            bd = json.loads(behaviors_path.read_text(encoding="utf-8"))
            current_behaviors = bd.get("behaviors", [])
        except:
            pass
    
    if current_behaviors:
        parts.append("<table><thead><tr><th>Behavior</th><th>Category</th><th>Span ID</th><th>Report ID</th></tr></thead><tbody>")
        for b in current_behaviors:
            value = b.get("value", "")
            category = b.get("category", "unknown")
            span_id = b.get("span_id", "")
            report_id = b.get("report_id", "")
            parts.append(f"<tr><td><strong>{esc(value)}</strong></td><td class='mono'>{esc(category)}</td><td class='mono'>{esc(span_id)}</td><td class='mono'>{esc(report_id)}</td></tr>")
        parts.append("</tbody></table>")
    else:
        parts.append("<p class='note'>No behavioral indicators extracted from current case. Run behavioral extraction to capture tactics and payment methods.</p>")
    
    parts.append("</div>")
    
    # Behavioral Families - memory-driven behavioral patterns
    parts.append("""<div class="section">
  <h2>👥 Behavioral Families</h2>
  <p class="small">Cross-case behavioral patterns showing how tactics and payment methods evolve. Families help identify campaign relationships.</p>
""")
    
    # Load memory index for behavioral families
    idx_path = ART / "memory" / "reuse_index.json"
    if not idx_path.exists():
        parts.append("<p class='note'>No behavioral families available. Build memory index to track behavioral evolution across cases.</p>")
        parts.append("</div>")
        return
    ...
    try:
        ...
        # Extract behavioral patterns (those with type 'behavior' or behavioral categories)
        behavioral_items = {}
        for k, rec in items.items():
            kinds = rec.get("kinds", [])
            categories = rec.get("categories", [])
            if ("behavior" in kinds or 
                any(cat in categories for cat in ["redirect", "spoof", "urgency", "payment"]) or
                any(term in k.lower() for term in ["behavior", "redirect", "spoof", "payment", "urgency"])):
                behavioral_items[k] = rec
        
        if behavioral_items:
            parts.append(f"<p><strong>Behavioral Families:</strong> {len(behavioral_items)} unique patterns across cases</p>")
            parts.append("<table><thead><tr><th>Behavioral Pattern</th><th>Family Size</th><th>Categories</th><th>Velocity</th><th>Cases</th></tr></thead><tbody>")
            
            # Sort by count (family size)
            sorted_behaviors = sorted(behavioral_items.items(), key=lambda x: x[1].get("count", 0), reverse=True)
            
            for k, rec in sorted_behaviors[:20]:  # Show top 20 behavioral families
                count = rec.get("count", 0)
                categories = rec.get("categories", [])
                cases = rec.get("cases", [])
                ...
                cases_str = ", ".join(cases[:3])
                if len(cases) > 3:
                    cases_str += f" (+{len(cases)-3})"
                
                # Extract pattern name from key
                pattern_name = k.split("::", 1)[1] if "::" in k else k
                
                parts.append(f"<tr><td class='mono'>{esc(pattern_name[:60])}</td><td>{count}</td><td class='small'>{esc(cats_str) if cats_str else '-'}</td><td>{velocity}</td><td class='small'>{esc(cases_str) if cases_str else '-'}</td></tr>")
            
            parts.append("</tbody></table>")
        else:
            parts.append("<p class='note'>No behavioral families detected in memory index. Process cases with behavioral extraction to build families.</p>")
    
    except Exception as e:
        parts.append(f"<p class='note'>Error reading behavioral families: {esc(str(e))}</p>")
...
<div class="header">
  <h1>🛡️ Safety Sigma v0.9b — Scripted (AUTHORITATIVE) + LLM (ADVISORY) + Proactive (EDAP) + Agentic + Memory + Behavioral</h1>
  ...
  <p class="small"><span class="pill">Scripted = authoritative (V-001..V-005)</span>
                 ...
                 <span class="pill">Memory = cumulative</span>
                 <span class="pill">Behavioral = literal</span></p>
</div>
...
""")
    ...
    # Behavioral Analysis section (v0.9b)
    render_behavioral_sections(parts)
...
<div class="section"><p class="small">Generated by Safety Sigma v0.9b · Scripted authoritative core + LLM/advisory + Proactive/EDAP + Backtesting + Behavioral Context + Living Knowledge Base + Behavioral Slice.</p></div>
</body></html>""")

    out = ART / "demo_report_v09b.html"
